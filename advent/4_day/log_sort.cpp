...
//        else cout << "Erro no parsing" << endl;
    ...
    for (it = horarios.begin(); it != horarios.end(); it++ ){
        ...
        if(it->second[60] > t_total){
            t_total = it->second[60];
            maior = it->first;
        }
    ...
    int maior_min, indice_maior;
    maior_min = 0;
    for(i = 0; i < 60; i ++){
        if(horarios[maior][i] > maior_min){
            maior_min = horarios[maior][i];
            indice_maior = i;
        }
    }


    int num_maior = find_num(maior);


    cout << "Guarda: " << num_maior << "  Minuto: " << indice_maior << endl;
    cout << num_maior * indice_maior << endl;

//    for(i = 0; i < valores.size(); i++){
//        log logui = valores[i];
//        cout << "data: " << logui.month << "\\" << logui.day << "   hora: " << logui.hour << ":" <<logui.minute << endl;
//    }
